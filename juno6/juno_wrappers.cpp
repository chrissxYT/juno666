...
void JunoKeyboard::midiGateChanged(int voiceNum, double data)
{
    ...
    if (data > 0)
        gtk_juno_keyboard_note_on(GTK_JUNO_KEYBOARD(widget),
            pitch_to_note(schedule->nyquistFreq * *inputs[voiceNum * 2].data) - 36);
    else
...
JunoSlider::JunoSlider(const char *name, Scheduler *sched, ConnectionManager *conn):
MoogObject(sched, conn)
{
...
JunoButton::JunoButton(GtkJunoButtonType type, const char *name, 
Scheduler *sched, ConnectionManager *conn, bool useLed):
MoogObject(sched, conn)
{
...
JunoSwitch::JunoSwitch(GtkJunoSwitchType type, const char *name, 
...
{
    int tmp = (int)data;
    gtk_juno_switch_set_pos(GTK_JUNO_SWITCH(widget), (GtkJunoSwitchPos)tmp);
}
