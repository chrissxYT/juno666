...
{
	numvoices = numVoices;
	dsp = new VSTOutput(control, schedule);

	lfo = new JunoLfo(control, numVoices, schedule);

	arpeggio = new JunoArpeggio(control, numVoices, schedule);

	pwmLfo = new Attenuator(schedule);

	noise = new Rand(schedule);

	voiceMix = new Mixer(schedule, numVoices);
	...
	for (int i = 0;i < numVoices;i++)
	{
		...
		voice[i] = new JunoVoice(control, i, schedule);
		char *tmp = new char[6];
		sprintf(tmp, "sig%d", i);
...
void VstJuno6::initProcess()
{
	...
	keyboard = new JunoKeyboard(numVoices, schedule);
